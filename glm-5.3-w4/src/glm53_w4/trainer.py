...
def rank_loss_scale(*, include: bool, world_size: int) -> float:
    ...
    FSDP averages gradients across all ranks.  Scaling each real sample by
    ``world_size`` makes a padded rank contribute exactly zero while preserving
    a sum over real samples; the accumulation boundary then divides by the
    global number of real samples.
    """
...
def accumulation_real_count(real_rank_counts: list[int] | tuple[int, ...]) -> int:
    """Return the denominator for an accumulation interval.

    The final interval can contain padded assignments on only some ranks.  Its
    denominator must therefore count real samples, not the number of loop
    iterations on each rank.
    """
...
def compute_dflash2_objective(
    *,
    ...
    if bool(hits.any()):
        ...
        selector = selector_numerator
    # Official DFlash2 uses two independently normalized terms: full-vocabulary
    # DFlash CE over every valid successor, plus selector CE only where the
    # ground-truth successor is already present in the base top-k candidates.
    total = base + selector
