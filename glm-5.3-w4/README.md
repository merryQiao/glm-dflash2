# GLM-5.3 W4A8: Stage B / Stage C

This is an isolated pipeline for the formal GLM-5.3 ModelSlim **W4A8** target
...
| quantization marker | ModelSlim W4A8 (`--quantization ascend`) |
...
its nested per-tensor entries for W4A8/W4A8_DYNAMIC metadata. A compact
...
SGLang rollout driver or another trusted GLM-5.3 W4A8 rollout producer. It must
...
non-W4A8 provenance. Sampling parameters belong to Stage A and are not silently
changed by Stage B; Stage B is a deterministic teacher-forced replay of the
stored token IDs.

## Stage B: vLLM-Ascend hidden extraction

Run this in the vendor vLLM-Ascend + ModelSlim W4A8 environment. The target
must be the exact same W4A8 checkpoint used for Stage A.

First export the frozen dense target input/output tensors:

```bash
cd /path/to/glm-dflash2/glm-5.3-w4

MODEL_PATH=/models/GLM-5.3-W4A8 \
...
that a particular vendor vLLM-Ascend build supports GLM-5.3 W4A8; that remains a
real-hardware gate.
