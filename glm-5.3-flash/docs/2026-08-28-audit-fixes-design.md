...
3. **DFlash2 supervision.** Use the real base top-k in both training and inference. Apply selector CE only to target-hit positions, with keep/depth weights masked by the hit indicator and a hit-only denominator. Never inject the target. Report unweighted unary top-k recall over valid positions separately.
...
- DFlash2 selector misses are excluded from its numerator and denominator without changing the candidate set.
