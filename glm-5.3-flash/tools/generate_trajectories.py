#!/usr/bin/env python3
"""Stage A: generate and freeze GLM-5.3-Flash coding-agent trajectories."""
...
from glm53_stage_a.agent_trajectory import (  # noqa: E402
...
from glm53_stage_a.sglang_stage_a import (  # noqa: E402
    AttemptErrorLedger,
...
from glm53_stage_a.jsonl import OutputShardLock, repair_truncated_jsonl  # noqa: E402
from glm53_stage_a.trajectory_tokens import freeze_trajectory_tokens  # noqa: E402
from glm53_stage_a.vibe_coding import (  # noqa: E402
    ModelInput,
    iter_table_rows,
    load_vibe_coding_table,
    row_to_model_input,
)
from glm53_stage_a.provenance import (  # noqa: E402
    dataset_fingerprint,
...
from glm53_stage_a.open_swe_trajectories import OpenSWETrajectoryStore  # noqa: E402
from glm53_stage_a.workspaces import (  # noqa: E402
    ORIGINAL_TRAJECTORY_KINDS,
    AutomaticWorkspaceProvider,
)
from glm53_stage_a.web_tools import (  # noqa: E402
    BROWSER_TOOL_NAME,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-path", type=Path, default=ROOT / "models/GLM-5.3-Flash-BF16"
    )
    parser.add_argument("--served-model-name", default="GLM-5.3-Flash-BF16")
    ...
    parser.add_argument("--reasoning-parser", default="glm45")
    parser.add_argument("--tool-call-parser", default="glm47")
...
def committed_route_summary(path: Path) -> dict[str, Any]:
    """Recompute route accounting exclusively from durable JSONL rows."""

    counts: Counter[str] = Counter()
    for number, row in enumerate(_iter_jsonl(path), 1):
        route = row.get("generation_route")
        if not isinstance(route, str) or not route:
            raise ValueError(f"{path}:{number} has no canonical generation_route")
        counts[route] += 1
    ...
    return {
        ...
        "counts": ordered,
        "fractions": {
            route: count / total for route, count in ordered.items()
        } if total else {},
    }
...
def _service_contract(
    args: argparse.Namespace, endpoint_identity: Mapping[str, Any] | None
) -> dict[str, Any]:
    """Return every server setting that can change generated trajectories."""

    if args.endpoint is not None:
        manifest = (
            endpoint_identity.get("manifest", {})
        ...
        return {
            ...
            "client_max_running_requests": args.max_running_requests,
            "dtype": manifest.get("dtype"),
            "tp_size": runtime.get("tp_size"),
            ...
            "deepep_mode": runtime.get("deepep_mode"),
    ...
    return {
        ...
        "weight_identity_status": "local_full_content_hash",
        "endpoint_identity": None,
    }


def _production_eligible(args: argparse.Namespace) -> bool:
    return not bool(args.allow_unverified_endpoint)


def _validate_endpoint_mode(args: argparse.Namespace) -> None:
    ...
    if args.allow_unverified_endpoint and args.endpoint_manifest is not None:
        raise ValueError(
            "--allow-unverified-endpoint and --endpoint-manifest are mutually exclusive"
        )
    if args.allow_unverified_endpoint and (
        args.max_samples is None or not 1 <= args.max_samples <= 50
    ):
        raise ValueError(
            "--allow-unverified-endpoint requires --max-samples in 1..50"
        )
    if args.endpoint is not None and args.endpoint_manifest is None:
        if not args.allow_unverified_endpoint:
            raise ValueError(
...
def _terminal_status(
    ...
    *,
    ...
    if args.allow_unverified_endpoint:
        if committed_count > 0 and unresolved_errors == 0:
            return "smoke_unverified"
        return "smoke_failed"
    if complete:
        return "frozen"
    return "partial" if args.max_samples is not None else "incomplete"


def _sample_budget_exhausted(
    *, max_samples: int | None, committed_count: int, attempted_count: int
) -> bool:
    """Treat max_samples as a total artifact budget, including prior resumes."""

    return (
        max_samples is not None
        and committed_count + attempted_count >= max_samples
    )
...
def _validate_resume_contract(
    ...
    """Reject resume whenever any immutable trajectory input has changed."""
...
def _web_contract(args: argparse.Namespace) -> dict[str, Any]:
    search_endpoint = args.web_search_endpoint or (
        DEFAULT_SEARXNG_ENDPOINT
        if args.web_search_provider == "searxng"
        else DEFAULT_SERPER_ENDPOINT
    )
    return {
        "input_kinds": getattr(args, "web_input_kind", None),
        "for_all": getattr(args, "web_tools_for_all", False),
        "search_provider": args.web_search_provider,
        ...
        "browser_max_calls": args.browser_max_calls,
        "api_key_file_sha256": (
            _file_digest(args.web_api_key_file) if args.web_api_key_file else None
        ),
    }
...
def _generate_one(
    *,
    ...
    trajectory["generation_route"] = route
...
def main() -> int:
    ...
    vocab_size = target_vocab_size(target_config)
    endpoint_identity = None
    if args.endpoint is not None:
    ...
    committed_before_run = len(done)
    truncate = args.no_resume
    run_contract = {
        "schema_version": 3,
        "stage": "trajectory",
        "status": "running",
        "production_eligible": _production_eligible(args),
        ...
        "served_model_name": args.served_model_name,
        "service": _service_contract(args, endpoint_identity),
    ...
    if manifest_path.exists() and not args.no_resume:
        ...
        _validate_resume_contract(previous, run_contract)
    _write_manifest(args.output_jsonl, run_contract)

    with tempfile.TemporaryDirectory(prefix="glm53-stage-a-") as temp_name:
        ...
        with _endpoint(args, temp_root) as endpoint, ExitStack() as stack, CommittedJsonlWriter(
            ...
            def commit(
                ...
                nonlocal accepted
                if trajectory.get("generation_route") != route:
                    raise ValueError("trajectory generation_route differs from commit route")
            ...
            def jobs() -> Iterator[tuple[int, str, ModelInput]]:
                ...
                for source_index, source_row in enumerate(iter_table_rows(selected)):
                    ...
                    if sample_id in done:
                        continue
                    if _sample_budget_exhausted(
                        max_samples=args.max_samples,
                        committed_count=committed_before_run,
    ...
    run_contract.update(
        {
            ...
            "extra_ids": len(extra),
            "route_summary": committed_route_summary(args.output_jsonl),
        }
    )
    _write_manifest(args.output_jsonl, run_contract)
    output_lock.__exit__(None, None, None)
    print(compact_json(run_contract), flush=True)
    if status in {"incomplete", "smoke_failed"}:
        raise RuntimeError(
