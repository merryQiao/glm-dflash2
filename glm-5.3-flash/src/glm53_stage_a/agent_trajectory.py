    ...
    def assert_token_id_capability(self) -> None:
        ...
        try:
            try:
                ...
                raise TrajectoryError(
                    "The inference endpoint cannot return the exact sampled token IDs "
                    "required by Stage A; use a runtime that returns prompt_token_ids "
                    f"and response_token_ids ({exc})"
                ) from exc
        finally:
