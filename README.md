...
trains its selector on a target-bearing candidate set: a base top-k miss is
replaced only for the training selector objective, while `unary_recall` is
measured before injection and inference top-k generation is unchanged. This
selector path remains experimental until the Ascend runtime parity gate is
implemented. DSpark follows its
