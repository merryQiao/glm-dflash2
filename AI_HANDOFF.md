...
提取、三种方法训练/FSDP2/checkpoint、带完整 frozen I/O 的标准 export、本地数值与训练
smoke，以及串行 target-only/speculative vLLM-Ascend benchmark 驱动。
...
embedding 和 LM head 必须来自同一不可变 GLM-5.2 BF16 revision。外部 endpoint 必须
提供 `endpoint-manifest.json` 并固定模型、tokenizer、BF16 精度、SGLang/CANN、镜像和 TP
身份；只有显式 smoke 才能使用 unverified endpoint。
...
RoPE theta                  8e6
RMS epsilon                 1e-5
block B8 / B16              1 anchor + 7 / 15 mask positions
```
...
- DSpark：plain backbone + rank-256 Markov head + confidence head；用 final hidden
  重建 target 分布，loss 为 `0.1 CE + 0.9 TV + 1.0 BCE`，其中
  `TV=0.5*sum_v|p_target(v)-p_draft(v)|`。

DSpark 在 target token 位置 `p` 使用 final hidden `p-1`。首位置 predecessor 是 clean
anchor，后面是 teacher-forced previous target。target embedding/lm-head 在训练时完全
冻结、不进入 optimizer；最终 deployment export 会把它们与 draft 合并，满足公开
DFlash/DSpark checkpoint 的加载契约。
...
Eval:    scripts/eval_vllm_ascend.sh
...
4. 在最终 Ascend serving 环境先回读 `export_manifest.json` 并校验 checksum。DFlash 和
   DSpark 用 stock vLLM-Ascend proposer 做 logits/acceptance gate；DFlash2 在自定义
   adapter 完成前不能标记为 stock runtime compatible。
5. 用 `scripts/eval_vllm_ascend.sh` 在相同卡、相同 prompt 上串行测 target-only 和
   speculative。温度 0 必须逐输出一致，并同时报告 bonus-inclusive acceptance、TPS、
   speedup；完成后才能宣称端到端可用。
