...
class AgentTrajectoryTest(unittest.TestCase):
    def test_token_id_probe_reports_generic_runtime_requirement(self):
        ...
        with patch.object(client, "complete", return_value={"_response_metadata": {}}):
            with self.assertRaises(TrajectoryError) as caught:
                client.assert_token_id_capability()
        message = str(caught.exception)
        self.assertIn("exact sampled token IDs", message)
        self.assertNotIn("0.5.16", message)
        self.assertNotIn("compatibility patch", message)
