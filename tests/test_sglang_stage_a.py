...
from tools.generate_trajectories import _existing_ids, parse_args
import tools.generate_trajectories as generate_trajectories

from glm53_stage_a.agent_trajectory import (
    ChatCompletionConfig,
    OpenAIChatClient,
    TrajectoryError,
)
from glm53_stage_a.sglang_stage_a import (
    AttemptErrorLedger,
...
class SGLangStageATest(unittest.TestCase):
    def test_route_summary_is_recomputed_from_only_committed_rows(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "trajectory.jsonl"
            rows = [
                {"id": "a", "generation_route": "original_trajectory"},
                {"id": "b", "generation_route": "workspace_task"},
                {"id": "c", "generation_route": "workspace_task"},
            ]
            path.write_text(
                "".join(json.dumps(row) + "\n" for row in rows), encoding="utf-8"
            )
            summary = generate_trajectories.committed_route_summary(path)
            self.assertEqual(summary["total"], 3)
            self.assertEqual(
                summary["counts"],
                {"original_trajectory": 1, "workspace_task": 2},
            )
            self.assertAlmostEqual(summary["fractions"]["workspace_task"], 2 / 3)

            # Resume/partial-error accounting is derived from durable rows, not
            # in-memory accepted counters.
            with path.open("a", encoding="utf-8") as handle:
                handle.write(json.dumps({"id": "d", "generation_route": "conversation_seed"}) + "\n")
            self.assertEqual(generate_trajectories.committed_route_summary(path)["total"], 4)

            # --no-resume truncation naturally removes old route counts.
            path.write_text(
                json.dumps({"id": "fresh", "generation_route": "conversation_seed"}) + "\n",
                encoding="utf-8",
            )
            self.assertEqual(
                generate_trajectories.committed_route_summary(path)["counts"],
                {"conversation_seed": 1},
            )

    def test_route_summary_rejects_committed_row_without_canonical_route(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "trajectory.jsonl"
            path.write_text('{"id":"a"}\n', encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "generation_route"):
                generate_trajectories.committed_route_summary(path)

    def test_glm53_server_command_uses_model_specific_defaults(self):
        command = build_server_command(
            SGLangServerConfig(
                python="/opt/python",
                model_path=Path("/models/GLM-5.3-Flash-BF16"),
                tp_size=16,
                host="127.0.0.1",
                port=30000,
            )
    ...
    def test_glm53_server_parsers_are_configurable(self):
        command = build_server_command(
            SGLangServerConfig(
                python="python",
                model_path=Path("/models/glm53"),
                reasoning_parser="custom-reasoning",
                tool_call_parser="custom-tools",
            )
        )
        self.assertEqual(
            command[command.index("--reasoning-parser") + 1], "custom-reasoning"
        )
        self.assertEqual(
            command[command.index("--tool-call-parser") + 1], "custom-tools"
        )

    def test_cli_exposes_model_specific_parsers(self):
        argv = [
        ...
        with patch.object(sys, "argv", argv):
            args = parse_args()
        self.assertEqual(args.reasoning_parser, "reasoning-x")
        self.assertEqual(args.tool_call_parser, "tools-y")
        self.assertEqual(args.served_model_name, "GLM-5.3-Flash-BF16")

    def test_glm53_server_command_exposes_optional_ascend_moe_settings(self):
        command = build_server_command(
            SGLangServerConfig(
                python="python",
                model_path=Path("/models/GLM-5.3-Flash-BF16"),
    ...
    def test_token_id_capability_probe_fails_before_rollout_when_server_omits_ids(self):
        ...
        client.session.post = Mock(return_value=response)
        with self.assertRaisesRegex(TrajectoryError, "exact sampled token IDs"):
