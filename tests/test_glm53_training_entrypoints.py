...
def test_training_launchers_are_valid_and_production_is_ascend_only() -> None:
    for relative in ("scripts/train_drafter.sh", "scripts/smoke_stage_c_training.sh"):
        path = ROOT / relative
    ...
    smoke = (ROOT / "scripts/smoke_stage_c_training.sh").read_text(encoding="utf-8")
    ...
    combined = "\n".join(
        ...
        for relative in (
            ...
            "scripts/smoke_stage_c_training.sh",
        )
    )
