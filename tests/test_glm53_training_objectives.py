...
def test_depth_weights_and_selector_loss_masks_candidate_misses() -> None:
    ...
    candidate_logits = torch.tensor(
        [[0.2, 1.1, -0.3], [9.0, 8.0, 7.0]], requires_grad=True
    )
    ...
    loss.mean.backward()
    assert torch.count_nonzero(candidate_logits.grad[0]) > 0
    assert torch.count_nonzero(candidate_logits.grad[1]) == 0


def test_selector_loss_all_misses_is_differentiable_zero() -> None:
    candidate_ids = torch.tensor([[4, 2, 1], [5, 6, 7]])
    candidate_logits = torch.randn(2, 3, requires_grad=True)
    targets = torch.tensor([8, 9])
    loss = selector_cross_entropy(candidate_ids, candidate_logits, targets)
    assert loss.denominator.item() == 0.0
    assert loss.mean.item() == 0.0
    loss.mean.backward()
    assert candidate_logits.grad is not None
    assert torch.count_nonzero(candidate_logits.grad) == 0


def test_dflash2_training_keeps_real_topk_and_reports_misses() -> None:
    ...
    assert torch.equal(training_ids, inference_ids)
    torch.testing.assert_close(training_logits, inference_logits)
    assert not training_ids[1].eq(targets[1]).any()
