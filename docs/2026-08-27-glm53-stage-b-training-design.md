...
- Rank-256, top-16 candidate selector. Training records base top-16 recall
  before intervention; on a miss only the weakest training candidate is
  replaced with the target so every positively weighted selector position has
  a valid local label. Inference candidate generation is unchanged.
- Convolution kernel size 2 and channel group size 16.
- Selector coefficient 1.0. The optimized global mean is
  `(base_numerator + selector_numerator) / base_denominator`; base and selector
  use the same keep/depth-weight denominator, while pre-injection
  `unary_recall` remains a separate diagnostic.
