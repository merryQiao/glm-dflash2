...
### Task 2: DFlash2 target-bearing candidate supervision
...
- [x] Add failing hit/miss candidate tests, pre-injection unary-recall assertions, keep/depth weighting, denominator equality, gradient, and inference-preservation tests.
- [x] Confirm miss test fails because target is absent/ignored.
- [x] Add training-only target injection and make the objective fail if a positively weighted target remains absent.
- [x] Keep inference candidate generation unchanged.
- [x] Run focused tests green.
