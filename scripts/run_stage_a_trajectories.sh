...
PY=${PY:-python}
MODEL_PATH=${MODEL_PATH:?set MODEL_PATH to the local GLM-5.3-Flash-BF16 checkpoint/tokenizer}
OUTPUT_JSONL=${OUTPUT_JSONL:-$ROOT/outputs/glm53_trajectories/trajectories-shard-${DATA_SHARD_INDEX:-0}-of-${DATA_SHARD_COUNT:-1}.jsonl}

args=(
  "$ROOT/tools/generate_trajectories.py"
  --dataset "${DATASET:-$ROOT/data/vibe_coding_630k}"
  --model-path "$MODEL_PATH"
  --served-model-name "${SERVED_MODEL_NAME:-GLM-5.3-Flash-BF16}"
