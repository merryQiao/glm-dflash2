...
def _scalar_values(value: Any) -> list[Any]:
    ...
    level key silently rejects valid W8A8 exports.
    """
...
def validate_w8a8_target_config(
    ...
    *,
    ...
    """Validate formal GLM-5.3 plus an explicit ModelSlim W8A8 marker.

    ``quant_description`` should be the parsed ModelSlim
    ``quant_model_description.json`` when validating a production export;
    ModelSlim generally does not put the W8A8 marker in ``config.json``.
    ...
    compatible = {
        ...
        if "w8a8" in value and not any(
            token in value for token in ("w8a8c8", "w4a8", "fp8")
        )
    }
    incompatible = {
        value
        for value in quantization
        if any(token in value for token in ("w8a8c8", "w4a8", "fp8"))
    }
    if incompatible or not compatible:
        raise ValueError(
            "formal GLM-5.3 checkpoint must carry ModelSlim W8A8 metadata; "
            f"found {sorted(quantization)}"
        )
    return {
        "schema": "formal-glm53-w8a8-v2",
