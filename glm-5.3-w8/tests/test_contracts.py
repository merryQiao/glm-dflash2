...
from glm53_w8.contracts import (
    DFLASH2_METHOD,
    DSPARK_METHOD,
    DRAFT_CONTRACT,
    TARGET_CONTRACT,
...
def test_w8a8_config_accepts_modelslim_metadata() -> None:
    metadata = validate_w8a8_target_config(_target_config())
    assert metadata["quantization"] == "W8A8"
    assert metadata["model_type"] == "glm_moe_dsa"


def test_w8a8_config_accepts_nested_modelslim_description() -> None:
    ...
    metadata = validate_w8a8_target_config(config, quant_description=description)
    assert metadata["quantization"] == "W8A8"


def test_w8a8_config_rejects_w8a8c8_and_mixed_w4a8() -> None:
    config = _target_config()
    config["quantization_config"] = {"model_quant_type": "W8A8C8"}
    with pytest.raises(ValueError, match="W8A8"):
        validate_w8a8_target_config(config)
    config["quantize"] = None
    config["quantization_config"] = {}
    with pytest.raises(ValueError, match="W8A8"):
        validate_w8a8_target_config(
...
def test_w8a8_config_rejects_other_targets(change: dict[str, object]) -> None:
    config = _target_config()
