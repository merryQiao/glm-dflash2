...
class CandidateSelector(nn.Module):
    ...
    def training_forward(
        ...
        """Return the real base top-k candidates and their target-hit mask.

        The selector is trained only where the frozen LM-head top-k already
        contains the target.  Training must therefore use exactly the same
        candidate set as inference; a missed target is never injected.
        """
