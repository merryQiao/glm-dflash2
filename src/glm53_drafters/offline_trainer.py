...
class OfflineMethodTrainer:
    ...
    def compute_loss(self, batch: TrainingBatch) -> TrainingResult:
        ...
        if self.method == "dflash2":
            ...
            # Match the official candidate_recall diagnostic: fraction of valid
            # supervised positions whose target is present in base top-k.  Loss
            # decay applies to the objectives, not to this recall metric.
            recall_mask = batch.keep_mask.float().unsqueeze(-1).expand_as(weights)
            unary_recall = _weighted_scalar(
                unary_hits[..., 1:].float(), recall_mask
            )
            # NeMo AutoModel DFlash2 optimizes two independently normalized
            # terms: base full-vocabulary CE + selector CE over top-k hits.
            # Represent their sum as one per-microbatch scalar so the existing
            # distributed accumulation averages valid microbatches without
            # incorrectly forcing the selector to share the base denominator.
            total_mean = base.mean + selector.mean
            valid_microbatch = (base.denominator > 0).to(dtype=torch.float32)
            total_numerator = total_mean * valid_microbatch
            total = AdditiveScalar(
                total_numerator,
                valid_microbatch,
                additive_mean(total_numerator, valid_microbatch),
            )
